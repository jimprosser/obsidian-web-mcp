...
def test_simultaneous_migration_is_idempotent(state_paths, monkeypatch):
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        results = list(pool.map(lambda _: migrate_once(), range(2)))
...
def test_authorization_code_expires_at_300_seconds_and_is_single_use(state_paths):
    ...
    assert issued.refresh_token.startswith("r1.")
...
def test_legacy_migration_is_one_way_and_secret_free(state_paths):
    _, state_path, legacy_path = state_paths
    records = {
        "first-client": {
            "client_secret": "first-secret-marker",
            "redirect_uris": ["https://first.example.test/cb"],
            "created_at": 1.0,
        },
        "second-client": {
            "client_secret": "second-secret-marker",
            "redirect_uris": ["https://second.example.test/cb"],
            "created_at": 2.0,
        },
    }
    ...
    assert not legacy_path.exists()
    assert state.get_client("first-client") is not None
    assert state.get_client("second-client") is not None
    assert "first-secret-marker" not in _db_text(state_path)
    assert "second-secret-marker" not in _db_text(state_path)
    state.close()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"client": {"client_secret": "s", "redirect_uris": "bad"}},
        {
...
def test_invalid_legacy_import_is_atomic(state_paths, records):
    _, state_path, legacy_path = state_paths
    _write_legacy(legacy_path, records)
    state = _state(state_paths)
...
def test_unlink_failure_fails_migrate_then_retries_without_reimport(
    ...
    assert len(state.list_clients()) == 1
    assert not legacy_path.exists()
    state.close()
...
def test_refresh_token_is_hashed_and_bound_to_its_access_token(state_paths):
    state = _state(state_paths)
    _, state_path, _ = state_paths
    client_id, secret = _register(state)
    issued = _redeem(state, _issue_code(state, client_id), client_id, secret)

    refresh = state.lookup_refresh_token(issued.refresh_token)
    assert refresh is not None
    assert refresh.client_id == client_id
    assert refresh.access_token_id == issued.token.token_id
    assert refresh.resource == RESOURCE
    assert refresh.consumed_at is None
    assert issued.refresh_token not in _db_text(state_path)
    assert state.lookup_refresh_token(issued.refresh_token + "wrong") is None
    assert state.lookup_refresh_token("v1.a.b") is None
    assert state.lookup_refresh_token("malformed") is None
    state.close()


def test_refresh_outlives_access_and_rotation_replaces_both(state_paths):
    now = [1_000.0]
    state = _state(state_paths, now=now)
    client_id, secret = _register(state)
    issued = _redeem(state, _issue_code(state, client_id), client_id, secret)

    now[0] = issued.token.expires_at + 1
    assert state.lookup_access_token(issued.access_token) is None

    rotated = _redeem_refresh(state, issued, client_id=client_id, client_secret=secret)
    assert rotated.access_token.startswith("v1.")
    ...
    with pytest.raises(InvalidGrant, match="no longer valid"):
        _redeem_refresh(state, issued, client_id=client_id, client_secret=secret)
    state.close()


def test_refresh_reuse_kills_its_session_access_token(state_paths):
    state = _state(state_paths)
    client_id, secret = _register(state)
    issued = _redeem(state, _issue_code(state, client_id), client_id, secret)
...
def test_refresh_expires_independently_of_access(state_paths):
    now = [1_000.0]
    state = _state(state_paths, now=now)
    client_id, secret = _register(state)
    issued = _redeem(state, _issue_code(state, client_id), client_id, secret)
    assert issued.refresh.expires_at == now[0] + 3_600

    now[0] = issued.refresh.expires_at
    with pytest.raises(InvalidGrant, match="expired"):
        _redeem_refresh(state, issued, client_id=client_id, client_secret=secret)
    state.close()


def test_refresh_rejects_resource_mismatch_and_missing_resource_is_inherited(
    state_paths,
):
    state = _state(state_paths)
    client_id, secret = _register(state)
    ...
    assert inherited.token.resource == RESOURCE
    state.close()


def test_client_revoke_and_access_revoke_kill_refresh(state_paths):
    state = _state(state_paths)
    first_id, first_secret = _register(state)
    second_id, second_secret = _register(state)
    first = _redeem(state, _issue_code(state, first_id), first_id, first_secret)
    second = _redeem(state, _issue_code(state, second_id), second_id, second_secret)

    assert state.revoke_token(first.token.token_id)
    assert state.lookup_refresh_token(first.refresh_token) is None
    with pytest.raises(InvalidGrant):
        _redeem_refresh(state, first, client_id=first_id, client_secret=first_secret)

    assert state.revoke_client(second_id)
    assert state.lookup_refresh_token(second.refresh_token) is None
    with pytest.raises(InvalidClient):
        _redeem_refresh(state, second, client_id=second_id, client_secret=second_secret)
    state.close()


def test_revoking_a_refresh_token_kills_its_access_token(state_paths):
    state = _state(state_paths)
    client_id, secret = _register(state)
    issued = _redeem(state, _issue_code(state, client_id), client_id, secret)
    refresh_id = issued.refresh.token_id

    assert state.revoke_refresh_token(refresh_id)
    assert not state.revoke_refresh_token(refresh_id)
    assert state.lookup_access_token(issued.access_token) is None
    assert state.lookup_refresh_token(issued.refresh_token) is None
    state.close()
