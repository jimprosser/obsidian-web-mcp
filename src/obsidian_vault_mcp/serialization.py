...
It is also the single place that controls how every tool response is encoded, so
all tool modules should route through dumps() rather than calling json.dumps
directly. Two defaults make responses token-efficient:

- ensure_ascii=False: non-ASCII text (Korean, Japanese, emoji, etc.) is emitted
  verbatim as UTF-8 instead of \\uXXXX escapes. The default True roughly tripled
  the size of any non-ASCII response and produced escaped paths that could fail
  to round-trip. The decoded object is identical either way; ASCII-only responses
  are unaffected.
