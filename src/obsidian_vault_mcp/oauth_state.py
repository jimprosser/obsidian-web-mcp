...
class UnsafeStatePath(OAuthStateError):
    """A state, migration, or output path violates local security rules."""
...
class OAuthState:
    ...
    def register_client(
        ...
        *,
        ...
        with self._transaction() as connection:
            ...
            connection.execute(
                "INSERT INTO clients "
                "(client_id, secret_verifier, is_static, client_name, "
                "created_at) VALUES (?, ?, 0, ?, ?)",
                (
                    client_id,
        ...
        *,
        ...
        with self._transaction() as connection:
            ...
            if existing is None:
                connection.execute(
                    "INSERT INTO clients "
                    "(client_id, secret_verifier, is_static, "
                ...
                connection.execute(
                    "UPDATE clients SET client_name = ? "
                    "WHERE client_id = ?",
        ...
        *,
        ...
        with self._transaction() as connection:
            client = connection.execute(
                "SELECT revoked_at FROM clients WHERE client_id = ?",
                (client_id,),
            ).fetchone()
        ...
        with self._lock:
            ...
            for row in rows:
                if not include_inactive and (
                    ...
                    continue
                tokens.append(_token_from_row(row))
        return tuple(tokens)

    def lookup_refresh_token(self, refresh_token: str) -> RefreshTokenMetadata | None:
    ...
    def list_refresh_tokens(
        ...
        *,
        include_inactive: bool = True,
    ) -> tuple[RefreshTokenMetadata, ...]:
        now = self._clock()
        with self._lock:
            rows = self._connection.execute(
                "SELECT r.*, c.revoked_at AS client_revoked_at "
                "FROM refresh_tokens AS r JOIN clients AS c USING (client_id) "
                "ORDER BY r.issued_at, r.token_id"
            ).fetchall()
            tokens = []
            for row in rows:
                if not include_inactive and (
                    ...
                    continue
                tokens.append(_refresh_from_row(row))
        return tuple(tokens)
    ...
    def redeem_refresh_token(
        ...
        *,
        ...
        token_id, secret = parsed
        now = self._clock()
        leaked_session_access: str | None = None
        issued: IssuedAccessToken | None = None
        with self._transaction() as connection:
            self._authenticate_client_in_transaction(
                connection, client_id, client_secret
            )
            ...
            if row["revoked_at"] is not None or row["consumed_at"] is not None:
                # A replayed rotated or revoked refresh token means the session
                # leaked. The kill runs in its own committed transaction below:
                # raising inside this one would roll the revocation back.
                leaked_session_access = str(row["access_token_id"])
            elif now >= float(row["expires_at"]):
                raise InvalidGrant("refresh token expired")
            else:
                stored_resource = str(row["resource"])
                if resource is not None and resource != stored_resource:
                    raise InvalidTarget("refresh token resource mismatch")
                consumed = connection.execute(
                    "UPDATE refresh_tokens SET consumed_at = ? "
                    "WHERE token_id = ? AND consumed_at IS NULL AND revoked_at IS NULL",
                    (now, token_id),
                )
                if consumed.rowcount != 1:
                    raise InvalidGrant("refresh token was already consumed")
                issued = self._insert_access_token(
                    connection,
                    client_id=client_id,
        ...
        if leaked_session_access is not None:
            with self._transaction() as connection:
                connection.execute(
                    "UPDATE access_tokens SET revoked_at = ? "
                    "WHERE token_id = ? AND revoked_at IS NULL",
                    (now, leaked_session_access),
                )
            raise InvalidGrant("refresh token is no longer valid")
    ...
    def revoke_refresh_token(self, token_id: str) -> bool:
        ...
        with self._transaction() as connection:
            linked = connection.execute(
                "SELECT access_token_id FROM refresh_tokens WHERE token_id = ?",
                (token_id,),
            ).fetchone()
            if linked is None:
                return False
    ...
    def _insert_access_token(
        ...
        *,
        ...
        connection.execute(
            ...
            "(token_id, secret_verifier, client_id, resource, "
        ...
        if with_refresh:
            while True:
                ...
                exists = connection.execute(
                    "SELECT 1 FROM refresh_tokens WHERE token_id = ?", (refresh_id,)
                ).fetchone()
                if exists is None:
                    break
            ...
            connection.execute(
                ...
                "(token_id, secret_verifier, client_id, access_token_id, "
                "resource, issued_at, expires_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
                (
                    refresh_id,
...
def _token_from_row(row: sqlite3.Row) -> TokenMetadata:
...
def _refresh_from_row(row: sqlite3.Row) -> RefreshTokenMetadata:
    return RefreshTokenMetadata(
        token_id=str(row["token_id"]),
        client_id=str(row["client_id"]),
        access_token_id=str(row["access_token_id"]),
        resource=str(row["resource"]),
        issued_at=float(row["issued_at"]),
        expires_at=float(row["expires_at"]),
        consumed_at=(
            float(row["consumed_at"]) if row["consumed_at"] is not None else None
        ),
        revoked_at=(
            float(row["revoked_at"]) if row["revoked_at"] is not None else None
        ),
