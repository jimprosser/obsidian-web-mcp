...
def initialize_oauth_state() -> OAuthState:
    ...
    Legacy `oauth_clients.json` is never imported here: migration runs only
    through the explicit `vault-mcp-oauth migrate` operator command.
    """
    global _oauth_state
    ...
    if _oauth_state is None:
        state = OAuthState(
            ...
            refresh_token_ttl_seconds=(config.oauth_refresh_token_ttl_seconds()),
...
def canonical_resource(request: Request) -> str:
    """RFC 8707 resource identifier: the actual MCP endpoint URL (#75).

    The base URL alone is right only when the transport is mounted at "/"; under
    VAULT_MCP_PATH the endpoint URL must carry that path, and strict clients
    (Home Assistant) reject metadata that disagrees with the URL they called.
    """
    base_url = config.advertised_base_url(str(request.base_url)).rstrip("/")
    if config.VAULT_MCP_PATH == "/":
        return base_url
    return f"{base_url}{config.VAULT_MCP_PATH}"


async def oauth_metadata(request: Request) -> JSONResponse:
    """OAuth authorization-server metadata (RFC 8414)."""
    base = config.advertised_base_url(str(request.base_url)).rstrip("/")
    return JSONResponse(
        {
            "issuer": base,
...
async def _handle_refresh_token(
    ...
    fields = {
        name: str(form.get(name, "") or "")
        for name in ("refresh_token", "client_id", "client_secret", "resource")
    }
    if not fields["refresh_token"] or not fields["client_id"] or not fields["client_secret"]:
        return _oauth_error(
            "invalid_request",
            "refresh_token, client_id, and client_secret are required",
        )
    resource: str | None = fields["resource"] or None
    try:
...
async def _handle_client_credentials(
    ...
    if resource != canonical_resource(request):
...
async def oauth_token(request: Request) -> JSONResponse:
    """Exchange a code, static client credentials, or a refresh token."""
    try:
    ...
    if grant_type == "refresh_token":
        return await _handle_refresh_token(request, form)
