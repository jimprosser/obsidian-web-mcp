...
# Legacy plaintext dynamic-client registry from earlier releases. Read only by the
# explicit `vault-mcp-oauth migrate` command, never by normal server startup. Override
# only to locate an existing deployment's source file.
...
_OAUTH_REFRESH_TOKEN_TTL_RAW = os.environ.get(
    "VAULT_OAUTH_REFRESH_TOKEN_TTL_SECONDS", "2592000"
)
...
def oauth_refresh_token_ttl_seconds() -> int:
    """Return the configured refresh-token TTL, bounded to 90 days."""
    try:
        ttl = int(_OAUTH_REFRESH_TOKEN_TTL_RAW)
    except (TypeError, ValueError) as exc:
        raise ValueError(
            "VAULT_OAUTH_REFRESH_TOKEN_TTL_SECONDS must be an integer "
            "between 1 and 7776000"
        ) from exc
    if not 1 <= ttl <= 7_776_000:
        raise ValueError(
            "VAULT_OAUTH_REFRESH_TOKEN_TTL_SECONDS must be between "
            "1 and 7776000"
        )
    return ttl
